...
v0.91 Improved prediction

v0.92 Fixed some crash, Collisions bug

v0.93 Some small improvements

Credits:

https://github.com/gamsteron

"""

from math import *
from valkyrie import *
from helpers.spells import BuffType, Buffs, CCType
...
enemies: list[ChampionObj] = []
minions: list[MinionObj] = []
...
def valkyrie_menu(ctx: Context):
    ...
    ui = ctx.ui
    ui.text("Ponche Prediction v0.93")
    ui.text("Work only with scripts that support it")
    ping = ui.sliderint("Ping", ping, 1, 200)
    extraDelay = ui.sliderint("Extra Delay", extraDelay, 0, 150)
...
class PredictionInput():
    def __init__(self, unit: UnitObj, fromPos, rangeCheckFrom, useBoundingRadius: bool = True, delay: float = 0, radius: float = 1, angle: float = 1, speed: float = hugeValue, range: float = hugeValue, type: SkillshotType = SkillshotType.SkillshotLine, collision: bool = False, collisionableObjects: list[CollisionableObjects] = []) -> None:
        self.From = fromPos
        self.RangeCheckFrom = rangeCheckFrom
        self.Unit = unit
        self.UseBoundingRadius = useBoundingRadius
        self.Delay = delay
        self.Radius = radius
        self.Angle = angle
        self.Speed = speed
        self.CollisionableObjects = collisionableObjects
...
class PredictionOutput():
    def __init__(self, unitPosition: Vec3, castPosition: Vec3, hitChance: HitChance = HitChance.Impossible, timeToHit: float = 0, aoeTargetsHitCount: int = 0, aoeTargetsHit: list[UnitObj] = [], collisionObjects: list[UnitObj] = []) -> None:
        self.TimeToHit = timeToHit
        self.UnitPosition = unitPosition
        self.CastPosition = castPosition
        self.HitChance = hitChance
        self.AoeTargetsHitCount = aoeTargetsHitCount
        self.AoeTargetsHit = aoeTargetsHit
...
class Tracker():
    def __init__(self, id: int) -> None:
        ...
        self.visibleTime: float = 0
        self.invisibleTime: float = 0
...
class Utility():
    ...
    def IsFacing(source: UnitObj, unit: UnitObj, angle: float = 90) -> bool:
    ...
    def GetPath(unit: UnitObj) -> list[Vec2]:
    ...
    def Extended(point: Vec2, direction: Vec2, extendRange: float) -> Vec2:
        if not direction:
            return point
        return Vec2(point.x + direction.x * extendRange, point.y + direction.y * extendRange)
    ...
    def GetImmobileDuration(target: ChampionObj) -> float:
    ...
    def GetInRange(fromPos: Vec3, enemies: list[ChampionObj], range: float) -> list[ChampionObj]:
        result = []
...
class Prediction():
    def GetPredictionData(target: UnitObj, source: Vec2, speed: float, delay: float, radius: float) -> tuple[Vec2, Vec2, float]:
    ...
    def IsHighHitChance(target: ChampionObj, index: int, duration: float) -> bool:
        tracker = trackers[index]
    ...
    def IsColliding(input: PredictionInput, source: Vec2, castPos: Vec2) -> bool:
        ...
        if Prediction.ShouldCheckThisCollision(collisions, CollisionableObjects.Minions):
            for minion in minions:
                if minion.visible and minion.net_id != unitId:
        ...
        if Prediction.ShouldCheckThisCollision(collisions, CollisionableObjects.Enemies):
            for enemy in enemies:
                if enemy.visible and enemy.net_id != unitId:
    ...
    def UpdateTracker(enemy: ChampionObj, index: int) -> None:
        global trackers
        if enemy.visible:
            if not trackers[index].visible:
                trackers[index].visible = True
                trackers[index].visibleTime = time()
        else:
            if trackers[index].visible:
                trackers[index].visible = False
...
class AoePrediction():
    ...
    class Candidate():
        def __init__(self, unit: UnitObj, pos: Vec2) -> None:
            self.Unit = unit
            self.Pos = pos
    ...
    def GetPossibleUnits(input: PredictionInput) -> list['Candidate']:
        ...
        result = []
        for enemy in enemies:
            if enemy.visible and enemy.net_id != originalUnitId and Utility.To2D(enemy.pos).distance(sourcePos) <= input.Range + input.Radius + enemy.bounding_radius:
                input.Unit = enemy
    ...
    def GetCandidatesUnits(candidates: list[Candidate]) -> list[UnitObj]:
        result = []
        for candidate in candidates:
...
def valkyrie_exec(ctx: Context):
    ...
    player = ctx.player
    enemies = ctx.champs.enemy_to(player).targetable().get()
    minions = ctx.minions.enemy_to(
        player).targetable().near(player, 2000).get()
